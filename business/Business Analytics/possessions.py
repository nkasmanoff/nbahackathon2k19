...
def possession_flagger(pbp_singlegame):
    ...
    pbp_singlegame['Poss Change Rebound'] = pbp_singlegame['Action_Type_Description'].apply(lambda z: True if z == 'Defensive' else False)
...
for game in pbp['Game_id'].unique()[0:1]:
    ...
    drebounds = rebounds.loc[rebounds['Action_Type_Description'] == 'Defensive']
    drebounds['Team_id'] = drebounds.apply(lambda z: teams[1] if z['Team_id'] == teams[0] else teams[0],axis=1)
    rebounds.loc[rebounds['Action_Type_Description'] == 'Defensive'] = drebounds
...
poop = pd.concat([here.shift(-2),pbp_singlegame[['Event_Msg_Type_Description','Action_Type_Description','score_x','score_y']],realpbp[3].shift(-2)],axis=1)