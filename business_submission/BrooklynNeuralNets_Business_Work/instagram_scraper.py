...
training_instas = pd.read_csv('training_set.csv',encoding = 'unicode_escape')
holdout_instas = pd.read_csv('holdout_set.csv',encoding = 'unicode_escape')
...
if 'tagged_profiles.csv' in os.listdir():
    
    profile_df = pd.read_csv('tagged_profiles.csv')
else:
    profile_df = load_tagged_profiles(instas)

    profile_df.to_csv('tagged_profiles.csv',index=False)
...
def load_hashtags(instas):
    ...
    try:
        ...
        print("Need selenium chromedriver set up in order to proceed with scraping. Please refer to selenium documentation to proceed.")
...
if 'used_hashtags.csv' in os.listdir():
    hashtag_df = pd.read_csv('used_hashtags.csv')
else:
    hashtag_df = load_hashtags(instas)

    hashtag_df.to_csv('used_hashtags.csv',index=False)
