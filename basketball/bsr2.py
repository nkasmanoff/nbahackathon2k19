...
def substitution_correction(subs,team_assignments):
    """
    
    The goal here is to fix the subs dataframe in order to confirm the assignment of a substitution to a team is 
    correct. 
    ...
    team_assignments : df
       Period = 0 from lineup data, the full rosters and assumed correct for both teams.
    ...
    team_assignments['Person1'] = team_assignments['Person_id']    
    
    del team_assignments['Period'],team_assignments['status'],team_assignments['Game_id']
    del subs['Team_id']
    
    
    first = pd.merge(subs, team_assignments,
        on = ['Person1'], how = 'left')
    #first.drop_duplicates(inplace=True)
    del team_assignments['Person1']
    ...
    if len(second.values) >0:
        first[nanmask] = second #and that's it.
...
def sub_correction(z,team_assignments):
    player = team_assignments.loc[team_assignments['Person_id'] == z['Person1']]
    try:
...
for game in pbp['Game_id'].unique()[0:1]:
   ...
   # print("Starters" , starting_lineup.loc[starting_lineup['Person_id'] == 'fbcda0bcb861e4726ca8871b8965ede4'])
    ...
    pbp_singlegame['Team_id'] = pbp_singlegame.apply(lambda z: sub_correction(z,team_assignments),axis=1)
...
lebron = starting_lineup.loc[starting_lineup['Person_id'] == 'fbcda0bcb861e4726ca8871b8965ede4']